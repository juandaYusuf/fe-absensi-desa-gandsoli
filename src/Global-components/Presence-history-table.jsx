...
const PresenceHistoryTable = ({ year, month }) => {
        ...
        <td key={day} className="fw-bold text-center text-success h4 border align-middle" style={{ backgroundColor: `${dayName === "Minggu" || dayName === "Sabtu" ? ("#ff000055") : (" ")}` }}>
          {
            dataPresence.map((result, i) => {
              ...
              if (result.presence_status === 'hadir') {
          ...
          <tbody >
            {
              attendanceList.map((resultOfAttendanceList, i) => {
                ...
                return (
                  <tr
                    className='align-middle cursor-pointer' key={resultOfAttendanceList.id}
                    // onClick={() => { presenceCounter(resultOfAttendanceList.id, fullname, presence) }}
                    >
